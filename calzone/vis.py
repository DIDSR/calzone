...
def plot_reliability_diagram(reliabilities, confidences, bin_counts, bin_edges=None, line=True, error_bar=False, z=1.96, title='Reliability Diagram', save_path=None, return_fig=False, custom_colors=None, dpi=150):
    ...
    for class_idx in range(num_classes):
        ...
        if line is True:
            # Plot as line
            plt.plot(class_confidences[mask], class_reliabilities[mask], 'x', color=colors[class_idx])
            plt.plot(class_confidences[mask], class_reliabilities[mask], '-', color=colors[class_idx], label='all' if num_classes == 1 else f'Class {class_idx}')
        else:
            # Plot as bar chart
            if bin_edges is None:
                bin_edges = np.linspace(0, 1, len(class_bin_counts)+1)  # assume equal spaced bin
            bin_centers = 0.5 * (bin_edges[:-1] + bin_edges[1:])
            width = 1/(len(bin_centers) * num_classes)
            plt.bar(bin_centers + width*class_idx, class_reliabilities, width=width, edgecolor='black', align='center', alpha=0.7, color=colors[class_idx], label='all' if num_classes == 1 else f'Class {class_idx}')
        ...
        if error_bar is not False:
            ...
            plus_minus = (z/(2*n))*(np.sqrt(4*n*p_hat*(1-p_hat) + z**2))*(1/(1 + (z**2)/n))
            ...
            plt.errorbar(class_confidences[mask], class_reliabilities[mask], yerr=score_interval, fmt='o', color=colors[class_idx], capsize=5, alpha=0.7)
...
def plot_roc_curve(fpr, tpr, roc_auc, class_to_plot=None, title='ROC Curve', save_path=None, dpi=150, return_fig=False):
